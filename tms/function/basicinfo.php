...
function GenerateBasicInfo($id)
{
  ...
  . "					<label>申请书提交时间</label>"
  . "					<input class=\"form-control date_input\" name=\"time\"  value=\"".$ans[9]."\">"
	. "					<label>发展阶段</label>";
...
function GetDangke($id)
{
	...
	echo("<input class=\"form-control\" name=\"jiben_dangke_qishu0\" value=".$ans[$num_qishu].">");
	echo("</div>");
	echo("<div class=\"col-md-6\">");
	//echo("<input class=\"form-control\" name=\"jiben_dangke_Pass0\" value=".$ans[$num_Pass].">");
	YesNo($ans[$num_Pass],"jiben_dangke_Pass0");
	echo("</div>");
	echo("</div>");
}	
